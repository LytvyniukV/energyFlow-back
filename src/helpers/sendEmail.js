...
const sendMailResetPassword = async (email, token) => {
  const mail = await sgMail.send({
    ...
    html: `<p>Please, follow the <a href="${SERVER_HOST}/api/users/reset-password/${token}">link</a> to reset your password</p>`,
    text: `Please, follow the link ${SERVER_HOST}/api/users/reset-password/${token} to reset your password`,
  });
...
const sendMailVerify = async (email, verifyToken) => {
  const mail = await sgMail.send({
    ...
    html: `<p>Please, follow the <a href="http://localhost:3001/api/users/verify/${verifyToken}">link</a> to verify your email</p>`,
